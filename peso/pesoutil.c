...
/*
* GetTimeHHMMSSFromDB()
* get time of day from db as a string Formatted HHMMSS
*/
char *GetTimeHHMMSSFromDB(char *szTimeString)
{
	PGresult *PGRes;
	PGRes=PGExecSQL(Cfg.nDebugVersion,"select to_char(CURRENT_TIMESTAMP,'HHMISS');");	
	if (PQresultStatus(PGRes) == PGRES_TUPLES_OK){
		strcpy(szTimeString,PQgetvalue(PGRes,0,0));
	}
	PQclear(PGRes);
...
/*
* GetDateYYYYMMDDFromDB()
* get time of day from db as a string Formatted HHMMSS
*/
char *GetDateYYYYMMDDFromDB(char *szTimeString)
{
	PGresult *PGRes;
	PGRes=PGExecSQL(Cfg.nDebugVersion,"select to_char(CURRENT_TIMESTAMP,'YYYYMMDD');");	
	if (PQresultStatus(PGRes) == PGRES_TUPLES_OK){
		strcpy(szTimeString,PQgetvalue(PGRes,0,0));
	}
	PQclear(PGRes);
