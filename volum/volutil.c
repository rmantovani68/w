...
void ReadConfiguration(BOOL bReadProcInfo)
{
	...
	*/
	...
	/*
	* Soglia per controllo barcode
	*/
	Cfg.nSogliaControlloBarcode = xncGetFileInt(szParagraph, "SogliaControlloBarcode", 0, szCniCfg, NULL);

	/*
	* spacchettamento
	*/
	Cfg.nNoUnpack = xncGetFileInt(szParagraph, "NoUnpack", 1, szCniCfg, NULL);
	Cfg.nPrelievoSingoloPacco = xncGetFileInt(szParagraph, "PrelievoSingoloPacco", 0, szCniCfg, NULL);
