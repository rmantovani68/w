...
int ReadImballi(PIMBALLI pImb)
{
	...
	*/
	PGRes=PGExecSQL(Cfg.nDebugVersion,"select codice, descriz, lunghezza, altezza, larghezza, tara, pesomax, volperc1, volperc2, tplavor from imballi where (larghezza > 0 AND altezza > 0 AND lunghezza > 0) OR tplavor = 1;");
	...
	for(nIndex=0;nIndex<nTuples;nIndex++){
		...
		pImb->Imballo[nIndex].nVolPerc[0] = atoi(PQgetvalue(PGRes,nIndex,7));
		pImb->Imballo[nIndex].nVolPerc[1] = atoi(PQgetvalue(PGRes,nIndex,8));
		/* tipo imballo - 0:linea automatica, 1:linea manuale 2:evasione su chiusura*/
		pImb->Imballo[nIndex].nSwLinea = atoi(PQgetvalue(PGRes,nIndex,9));
		...
		*/
		pImb->Imballo[nIndex].nVolumeUtile[0] = pImb->Imballo[nIndex].nVolume * (float)((float)pImb->Imballo[nIndex].nVolPerc[0]/(float)100);
		pImb->Imballo[nIndex].nVolumeUtile[1] = pImb->Imballo[nIndex].nVolume * (float)((float)pImb->Imballo[nIndex].nVolPerc[1]/(float)100);
...
int CmpImballo(PIMBALLO pImb1,PIMBALLO pImb2)
{
	...
	*/
	...
	/* 
	* imballo 1 piu' piccolo di imballo 2 per volume 
	*/
	if( pImb1->nVolume <= pImb2->nVolume){
		return -1;
	}
	/* 
	* imballo 1 piu' grande di imballo 2 per volume 
	*/
	if( pImb1->nVolume > pImb2->nVolume){
		return 1;
	}
...
#ifdef TRACE
						trace_out_vstr(1,"   %2d %-5.5s %7.2f %6d   %2d%%   %2d%% - %2d%%",
							Ordine.nColli-1,
							...
							Imballi.Imballo[Collo.nIndiceImballo].nVolPerc[0],
							Imballi.Imballo[Collo.nIndiceImballo].nVolPerc[1]);
#endif
				...
				*/
				...
				while(!bFine){
					...
					*/
					...
					if( pCollo->nVolume+pRiga->nVolumeCopia*(nRigaCopieCollo+max(1,pRiga->nPRQTKAR)) > Imballi.Imballo[pCollo->nIndiceImballo].nVolumeUtile[Cfg.nIsolaAttiva]){
				...
				if(nRigaCopieCollo==pRiga->nCopie){
					...
					*/
					...
					trace_out_vstr(1,"PP-P:[%s]: %3d copie %4dx%4dx%4d = %6d - V.C: %8d V.U: %8d",
						...
						pCollo->nVolume,Imballi.Imballo[pCollo->nIndiceImballo].nVolumeUtile[Cfg.nIsolaAttiva]);
#endif
			...
			if(pRiga->nProdottoInPiedi){
				...
				*/
				if( Imballi.Imballo[pCollo->nIndiceImballo].nHeight<pRiga->nHeight ||
						pCollo->nVolume+pRiga->nVolumeCopia*(nRigaCopieCollo+max(1,pRiga->nPRQTKAR)) > Imballi.Imballo[pCollo->nIndiceImballo].nVolumeUtile[Cfg.nIsolaAttiva]){
	#ifdef TRACE
		...
		while(!bFine && pCollo->nIndiceImballo<Imballi.nImballiAutomatici){
			...
			*/
			...
			if(pRiga->nProdottoInPiedi){
				...
				*/
				if( Imballi.Imballo[pCollo->nIndiceImballo].nHeight<pRiga->nHeight ||
						pCollo->nVolume+pRiga->nVolumeCopia*(nRigaCopieCollo+1) > Imballi.Imballo[pCollo->nIndiceImballo].nVolumeUtile[Cfg.nIsolaAttiva]){
	#ifdef TRACE
					trace_out_vstr(0,"... NON CI STA");
	#endif
