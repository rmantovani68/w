...
void ReadConfiguration(BOOL bReadProcInfo)
{
	...
	*/
	...
	/* gestione badge */
	Cfg.nGestioneBadge = xncGetFileInt(szParagraph, "GestioneBadge", 0, szCniCfg, NULL);
	...
	Cfg.nAttesaPDFOrdineCompleto=xncGetFileInt(szParagraph,"AttesaPDFOrdineCompleto",1,szCniCfg,NULL);
	...
	*/
	for(nIndex=0;nIndex<Cfg.nNumeroDisplays;nIndex++){
		...
		sscanf(szProcBuffer, "%d,%d,%d,%d,%d,%d",
...
BOOL GetOrdine(PORDINE pOrdine,int nIsola)
{
	...
	/*
	 * 2012-10-29: Ormain i PDF arrivano abbastanza puntuali quindi la priorita' di produzione
	 * ridiventa la coerenza di lancio dell'intero ordine. Quindi aspetto ancora il pdf ma non
	 * per lanciare solo l'ultimo collo ma tutti i colli dell'ordine.
	 */
	if (Cfg.nAttesaPDFOrdineCompleto) {
		/*
		 * Aspetto il pdf e lancio l'intero ordine
		 * 
		 */
		PGRes=PGExecSQL(Cfg.nDebugVersion>2,"select opstato,opcdisl,ordkey,opnelab,opnmcll,opnmrgh,opnmcpe,opflpdf,opflftz from ord_prod where ((opstato='%c' AND opflpdf!='%c') or (opstato='%c' AND opflpdf!='%c')) AND opcdisl=%d AND opnmcla>0  order by opprgpr,opprgln,ordkey;",ORDINE_ELABORATO,ORDINE_IN_ATTESA_PDF,ORDINE_INCOMPLETO_TERMINATO_SPEDITO,ORDINE_IN_ATTESA_PDF,nIsola);
	} else {
		/*
		 * Non aspetto per forza il pdf e lancio tutti i colli dell'ordine tranne l'ultimo
		 * 
		 */
		/*
		 * TODO
	...
	*/
	...
	switch(cStato){
		case ORDINE_IN_PRELIEVO:
		{
			...
			* Query originale
			*/
			PGRes=PGExecSQL(Cfg.nDebugVersion,"select r.ordkey,r.rpcdpro,r.rpcdubi,r.rpqtspe,r.rpprrig,r.rpnmcol,0,0,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore,r.rpprogr from rig_prod as r, ubicazioni as u where ordkey='%s' and u.ubicazione=r.rpcdubi and u.isola=%d order by u.priorita,r.rpnmcol,r.rpprrig;",pOrdine->szORDKEY,pSettore->nIsola);
		}
		break;
		...
		*/
		if(pSettore->nSettore==atoi(PQgetvalue(PGRes,nIndex,12))){
			...
			if(Cfg.nChiusuraCollo){
				...
				pSettore->RigaOrdine[nRigaIndex].nRigProg          = atoi(PQgetvalue(PGRes,nIndex,13));
				/*
	...
	*/
	if(nRigaIndex){
		...
		*/
		...
		pSettore->nRigProg=pRigaOrdine->nRigProg;
		...
		/*
		# Il Campo UBICAZIONE e' diviso in : ISSFLLR
		# I : ISOLA
		# S : SETTORE
		# F : FRONTE
		# L : LOCAZIONE
		# R : RIPIANO
		*/
		pSettore->nFronte=atoi(SubStr(pSettore->szCodUbi,3,1));
		pSettore->nOpBlinking=0;

		/*
		* rm 19-03-2013 : gestione per display 
		*/
		PGResProdotto=PGExecSQL(Cfg.nDebugVersion,"select prdescr,prflgpp,prperic from articoli where prcdpro='%s';",pSettore->szCodProd);
		if((nTuples=PQntuples(PGResProdotto))){
			strcpy(pSettore->szTitoloProd,PQgetvalue(PGResProdotto,0,0));
			pSettore->nInPiedi=atoi(PQgetvalue(PGResProdotto,0,1));
			pSettore->nPericoloso=atoi(PQgetvalue(PGResProdotto,0,2));
		} else {
			strcpy(pSettore->szTitoloProd,"-");
			pSettore->nInPiedi=0;
		}
		PQclear(PGResProdotto);
		...
		*/
		...
		/*
		* LG 2015-08-02 : gestione per controllo barcode identificativo BRI 
		*/
		PGResProdotto=PGExecSQL(Cfg.nDebugVersion,"select prswbri,rpflbri,rpqtbri from articoli,rig_prod where prcdpro=rpcdpro and prswbri='%c' and ordkey='%s' and rpnmcol=%d and rpcdpro='%s';",CONTROLLO_BARCODE_ON,pSettore->szCodSped,pSettore->nNumCollo,pSettore->szCodProd);
		if((nTuples=PQntuples(PGResProdotto))){
			//strcpy(pSettore->szFlagBarcodeId,PQgetvalue(PGResProdotto,0,1));
			sprintf(pSettore->szFlagBarcodeId,"%s",PQgetvalue(PGResProdotto,0,1));
			pSettore->nQtBarcodeId=atoi(PQgetvalue(PGResProdotto,0,2));
#ifdef TRACE
			trace_out_vstr_date(1,"BRI    Spedizione: [%s] Collo: [%d] Prodotto: [%s] FlagBRI: [%s] CopieBRI [%d]", pSettore->szCodSped,pSettore->nNumCollo,pSettore->szCodProd,pSettore->szFlagBarcodeId,pSettore->nQtBarcodeId);
			trace_out_vstr_date(1,"BRI    Copie: [%d] ",pSettore->nNumCopie );
#endif
			if (pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_OK || pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_FORZATO_PARZIALE || pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_DISABILITATO) {
#ifdef TRACE
				trace_out_vstr_date(1,"BRI    Nessun Controllo da fare");
#endif
				/*
				...
				*/
				...
				pSettore->nOpBlinking=0;
			} else if (pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_INCOMPLETO) {
#ifdef TRACE
				trace_out_vstr_date(1,"BRI    Entro in controllo barcode per flag = I");
#endif
				/*
				* setto lo stato settore in attesa di lettura barcodeid 
				*/
				if (pSettore->nQtBarcodeId>=pSettore->nNumCopie) {
#ifdef TRACE
				trace_out_vstr_date(1,"BRI    qt bri > qt");
#endif
					/*
					* setto lo stato settore in attesa di conferma copie prelevate
					*/
					...
					* setto lo stato settore in attesa di lettura barcodeid
					*/
					pSettore->nStatoSettore=LETTURA_BARCODE_ID;
					pSettore->nOpBlinking=0;
				}
			} else {
#ifdef TRACE
				trace_out_vstr_date(1,"BRI    altro flag");
#endif
				/*
				* setto lo stato settore in attesa di conferma copie prelevate
				*/
						...
						*/
						...
						if(cOPFLFTZ!=ORDINE_FITTIZIO && (Ordine.nColli==0 || Ordine.nRighe==0 || Ordine.nCopie==0)){
							...
							*/
							...
							/*
							 * Lancio di ordine senza ultimo collo 
							 */
								switch(cStato){
									case ORDINE_ELABORATO:
									{
		...
		*/
		if((pOrdine=link_first(&(ListaOrdini[nSettoreIndex])))!=NULL){
			...
			if(pSettore->nTipoSettore==SETTORE_START && Cfg.nImballiUbicati==1){
				...
				*/
				/*
				PGRes=PGExecSQL(FALSE,"select c.ordprog,c.ordkey,c.cpnmcol,c.cptpfor,u.ios,u.cpu,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore from col_prod as c, ubicazioni as u where ordkey='%s' and c.cpswlin!='1' and u.codprod=c.cptpfor and u.isola=%d order by c.cpnmcol;",pOrdine->szORDKEY,pSettore->nIsola);
				*/

				/*
				* Vecchia query usata prima della gestione pdf
				*/
				//PGRes=PGExecSQL(Cfg.nDebugVersion,"select c.ordprog,c.ordkey,c.cptpfor,count(c.cptpfor),u.ios,u.cpu,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore from col_prod as c, ubicazioni as u where ordkey='%s' and c.cpswlin!='1' and u.codprod=c.cptpfor and u.isola=%d group by c.cptpfor,c.ordprog,c.ordkey,u.ios,u.cpu,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore,u.priorita order by u.priorita;",pOrdine->szORDKEY,pSettore->nIsola);
				...
				*/
				...
				switch(cStato){
					case ORDINE_IN_PRELIEVO:
					{
						...
						* Query originale
						*/
						PGRes=PGExecSQL(Cfg.nDebugVersion,"select c.ordprog,c.ordkey,c.cptpfor,count(c.cptpfor),0,0,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore,u.ubicazione,c.cpflisp from col_prod as c, ubicazioni as u where ordkey='%s' and c.cpswlin!='1' and u.codprod=c.cptpfor and u.isola=%d group by c.cptpfor,c.ordprog,c.ordkey,u.settore_can,u.riga_can,u.colonna_can,u.display,u.settore,u.ubicazione,u.priorita,c.cpflisp order by u.priorita;",pOrdine->szORDKEY,pSettore->nIsola);
					}
					break;
					case ORDINE_INCOMPLETO_IN_PRELIEVO:
					{
...
***************************************************************************************************/
				...
				/*
				* Riempio la struttura IMBALLO per tutti i colli recuperati con la query
				*/
				nTuples=PQntuples(PGRes);
				for(nIndex=0;nIndex<nTuples;nIndex++){
					...
					strcpy(pImballo->szCodUbi,            PQgetvalue(PGRes,nIndex,11));
					strcpy(pImballo->szImballoSpeciale,   PQgetvalue(PGRes,nIndex,12));
				}
				PQclear(PGRes);

				/*
				* Riempio la struttura SETTORE con i dati recuperati dalla query
				*/
				...
				strcpy(pSettore->szCodProd,    pImballo->szFormato);
				strcpy(pSettore->szTitoloProd, "");
				strcpy(pSettore->szCodUbi,     pImballo->szCodUbi);
				strcpy(pSettore->szImballoSpeciale,     pImballo->szImballoSpeciale);
	...
	switch(pEvento->nEvento){
		...
		case BARCODE_ID_PRODOTTO:
		{
			switch(pSettore->nStatoSettore){
				...
				{
					/*
					* sono in attesa lettura barcode id
					* - se si tratta dell'ultima lettura di quelle previste devo impedirne altre
					* - se rileggo un barcode gia' letto 
					*/
					// Comincio col copiare in Settori il barcode letto, qualunque esso sia
					strcpy(pSettore->szBarcodeIdLetto,pEvento->szBarcode);
#ifdef TRACE
					trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: inizio");
					trace_out_vstr_date(1, "Barcode: [%s]",pSettore->szBarcodeIdLetto);
					trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: barcode arrivato");
#endif
					if (pSettore->nQtBarcodeId>=pSettore->nNumCopie){
#ifdef TRACE
						trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: barcode non gestito perche' qt superata");
#endif
						// mettere la stringa di lettura a ERRORE e ritornare alla fase di prelievo copie. Opure uscirne con pressione tasto?
						sprintf(pSettore->szFlagBarcodeId,"%c",CONTROLLO_BARCODE_OK);
						strcpy(pSettore->szBarcodeIdLetto,BARCODE_QT_SUPERATA);
					} else if ((strlen(pEvento->szBarcode))!=BARCODE_ID_LENGTH){
#ifdef TRACE
						trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: barcode errato per lunghezza");
#endif
						// mettere la stringa di lettura a ERRORE e ritornare alla fase di lettura barcode
						//strcpy(pSettore->szBarcodeIdLetto,BARCODE_ERRORE);
						strcpy(pSettore->szBarcodeIdLetto,BARCODE_LUNGH_ERRATA);
					} else if (pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_OK || pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_FORZATO_PARZIALE || pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_DISABILITATO) {
#ifdef TRACE
						trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: barcode non gestito perche' non richiesto");
#endif
					} else if (!strcmp(pEvento->szBarcode,BARCODE_CHIUDI_LETTURE)){
#ifdef TRACE
						trace_out_vstr_date(1, "FASE LETTURA_BARCODE_ID: barcode di chiusura letture");
#endif
						// mettere la stringa di lettura a Chiudi letture? e ritornare alla fase di prelievo copie?
						sprintf(pSettore->szFlagBarcodeId,"%c",CONTROLLO_BARCODE_FORZATO_PARZIALE);
					} else {
						int nIndex;
...
#ifdef TRACE
						trace_out_vstr_date(1, "lettura barcode id prodotto isola: %d settore: %d barcode: %s",pEvento->nIsola,pEvento->nSettore,pEvento->szBarcode);
#endif
						for(nIndex=0;nIndex<Cfg.nNumeroSettori;nIndex++){
							if(pSettori[nIndex].nIsola==pEvento->nIsola && pSettori[nIndex].nSettore==pEvento->nSettore ){
								/* eseguo le prodedure per lettura del barcode di identificazione*/
								...
								PGRes=PGExecSQL(Cfg.nDebugVersion,"insert into letture_barcode_id_prodotto (lbcdbri , lbtmlet , lbnisol , lbnsett , lbcdpro , ordkey , lbnmcol , lbnmcpe , lbdstit , lbcodop , lbnomop , lbcogop ) \
									...
									pEvento->nIsola,
									pEvento->nSettore,
									pSettori[nIndex].szCodProd,
									pSettori[nIndex].szCodSped,
									pSettori[nIndex].nNumCollo,
									pSettori[nIndex].nNumCopie,
									pSettori[nIndex].szTitoloProd,
									pSettori[nIndex].szCodOperatore,
									pSettori[nIndex].szNomeOperatore,
									pSettori[nIndex].szCognomeOperatore
								);
								if(PQresultStatus(PGRes) == PGRES_COMMAND_OK){
								} else {
									strcpy(pSettori[nIndex].szBarcodeIdLetto,BARCODE_ERRORE);
								}
					...
					if(PQresultStatus(PGRes) == PGRES_TUPLES_OK){
						pSettore->nQtBarcodeId=atoi(PQgetvalue(PGRes,0,0));
					} else {
					}
					PQclear(PGRes);
					...
					// Aggiorno rig_prod
					PGRes=PGExecSQL(Cfg.nDebugVersion,"update rig_prod set rpflbri='%c',rpqtbri=%d where rpcdpro='%s' and ordkey='%s' and rpnmcol=%d and rpprogr=%d",
						pSettore->szFlagBarcodeId[0],
						pSettore->nQtBarcodeId,
						pSettore->szCodProd,
						pSettore->szCodSped,
						pSettore->nNumCollo,
						pSettore->nRigProg
					...
					/* 
					* se ho terminato il controllo Barcode ID passo al prelievo copie 
					*/
					if (pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_OK || 
					    pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_FORZATO_PARZIALE || 
							pSettore->szFlagBarcodeId[0]==CONTROLLO_BARCODE_DISABILITATO) {
		...
		case IOS_PKL_BUTTON_PRESSED:
			switch(pSettore->nStatoSettore){
				case LETTURA_BARCODE_ID:
				{
					/*
					* sono in attesa lettura barcode id
					* se tasto premuto resetto e riaccendo
					*/
					/* reset lampada */
					ResetLampada(pSettore->nIOS,pSettore->nCPU,pSettore->nModulo,pSettore->nRiga,pSettore->nColonna);
					/* set lampada */
					SetLampada(pSettore->nIOS,pSettore->nCPU,pSettore->nModulo,pSettore->nRiga,pSettore->nColonna);
				}
				break;
									...
									if((pOrdine=link_first(&(ListaOrdini[nSettoreIndex])))!=NULL){
										...
										*/
										while(pSettori[nSettoreIndex].nIsola==pSettore->nIsola && pSettori[nSettoreIndex].nStatoSettore==DISABILITATO){
											nSettoreIndex++;
#ifdef TRACE
											trace_out_vstr_date(1,"incremento settoreIndex [%s-%2d]", pOrdine->szORDKEY,nSettoreIndex);
#endif
										}

										link_add(&(ListaOrdini[nSettoreIndex+1]),pOrdine);
#ifdef TRACE
										trace_out_vstr_date(1,"Passo Ordine [%s] a Settore [%2d]", pOrdine->szORDKEY,nSettoreIndex+1);
#endif
									}
								}
...
#ifdef TRACE
											trace_out_vstr_date(1,"BRI    Entro in controllo barcode per falg = I");
#endif
											/*
											* setto lo stato settore in attesa di lettura barcodeid 
											*/
...
BOOL GestioneBadge(EVENTO *pEvento)
{
	...
	PGresult *PGResProdotto = NULL;
	int nTuples;
	BOOL bOperatore;        /* codice operatore corretto in anagrafica */
	BOOL bOperatoreSettore; /* codice operatore corretto in settore */
	char szMsgTxt[256];
	char cStato;
	int nIndex;
	int nSettoreIndex;
	...
	if((nSettoreIndex=GetSettore(pEvento->nIsola,pEvento->nSettore))==-1){
#ifdef TRACE
		trace_out_vstr_date(1,"GestioneBadge(I:%d,S:%d) : settore non presente !",pEvento->nIsola,pEvento->nSettore);
#endif
		return FALSE;
	}
...
#ifdef TRACE_ELIMINATO
	trace_out_vstr_date(1,"GestioneBadge(I:%d,S:%d) : settore %d",pEvento->nIsola,pEvento->nSettore,nSettoreIndex);
#endif
	...
	switch(pEvento->nEvento){
		...
		case BADGE_ID:
		{
			PGresult *PGRes;
#ifdef TRACE
			trace_out_vstr_date(1, "Settore %02d isola %d - Arrivato BADGE_ID : '%s' - gestione badge : %d",pEvento->nSettore, pEvento->nIsola, pEvento->szBarcode, Cfg.nGestioneBadge);
#endif
			
			strcpy(szStatoLettura,"ERROR-UNDEFINED");

			if(Cfg.nGestioneBadge){

				nAttesaBadge = pSettore->nAttesaBadge;
				nOpBlinking  = pSettore->nOpBlinking;

				/*
				* il barcode operatore e' qualcosa come questo
				*	%##12?
				*	controllare che la lunghezza del barcode sia sufficiente
				*/
				if(strlen(pEvento->szBarcode) >= 4){
					szCDOPE[0]=pEvento->szBarcode[0];
					szCDOPE[1]=pEvento->szBarcode[1];
					szCDOPE[2]=pEvento->szBarcode[2];
					szCDOPE[3]=pEvento->szBarcode[3];
					szCDOPE[4]='\0';

					/* verifico codice operatore */
					PGRes=PGExecSQL(Cfg.nDebugVersion,"select opcdope from operatori where opcdope='%s';", szCDOPE);
					nTuples=PQntuples(PGRes);
					bOperatore=nTuples==1?TRUE:FALSE;
					PQclear(PGRes);
					
					/*
					* se sono in attesa badge 
					* esco dall'attesa 
					* se invece non ci sono e il badge corrisponde all'operatore attivo allora esco e vado in attesa badge
					*/
					if(pSettore->nAttesaBadge){



						if(bOperatore){
							PGRes=PGExecSQL(Cfg.nDebugVersion,"update opeset set opcdope='%s',dtmod='%s',tmmod='%s' where opisola=%d and opset=%d;", 
								szCDOPE,
							...
							sprintf(szMsgTxt,"Isola:%d : Settore:%d : Lettura badge [%s]\n",
								pEvento->nIsola, pEvento->nSettore,szCDOPE);
							SendMessage(PROC_MAIN, PROC_SETTORI, DISPLAY_MSG, szMsgTxt);
							
							/* esco da attesa badge */
							nAttesaBadge = FALSE;
							nOpBlinking  = 0;
							strcpy(pSettore->szCodOperatore,szCDOPE);
							PGRes=PGExecSQL(Cfg.nDebugVersion ,"select opnmope,opcgope from operatori where opcdope='%s';",szCDOPE);
							nTuples=PQntuples(PGRes);
							if(nTuples){
								strcpy(pSettore->szNomeOperatore,PQgetvalue(PGRes,0,0));
								strcpy(pSettore->szCognomeOperatore,PQgetvalue(PGRes,0,1));
							}
							PQclear(PGRes);

							strcpy(szStatoLettura,"ENTRATA - OK");
						} else {
							sprintf(szMsgTxt,"ATTENZIONE : Isola:%d : Settore:%d : Lettura badge errata [%s]\n",
								pEvento->nIsola, pEvento->nSettore,pEvento->szBarcode);
							SendMessage(PROC_MAIN, PROC_SETTORI, DISPLAY_MSG, szMsgTxt);
							strcpy(szStatoLettura,"ERROR-OPCODE NOT CORRECT");
						}
					} else {
						/* non sono in attesa badge */
						if(bOperatore){
							/* controllo che il codice operatore attuale sia lo stesso */
							PGRes=PGExecSQL(Cfg.nDebugVersion,"select opcdope from opeset where opcdope = '%s' and opisola=%d and opset=%d;", szCDOPE, pEvento->nIsola, pEvento->nSettore );
							nTuples=PQntuples(PGRes);
							bOperatoreSettore=nTuples==1?TRUE:FALSE;
							PQclear(PGRes);
						}

						if(bOperatoreSettore){
							pSettore->nAttesaBadge=TRUE;
							pSettore->nOpBlinking=1;
							PGRes=PGExecSQL(Cfg.nDebugVersion,"update opeset set opcdope='0000',dtmod='%s',tmmod='%s' where opisola=%d and opset=%d;", 
								GetDateYYYYMMDD(time((long *)0),szDateBuffer),
								GetTimeHHMMSS(time((long *)0),szTimeBuffer),
								pEvento->nIsola, pEvento->nSettore );
							PQclear(PGRes);

							ReadOperatori();
							strcpy(szStatoLettura,"USCITA - OK");
						} else {
							PGRes=PGExecSQL(Cfg.nDebugVersion,"select opcdope from opeset where opisola=%d and opset=%d;", pEvento->nIsola, pEvento->nSettore );
							nTuples=PQntuples(PGRes);
							if(nTuples){
								/* */
								sprintf(szMsgTxt,"ATTENZIONE : Isola:%d : Settore:%d : operatore [%s] non corrisponde a [%s]\n",
									pEvento->nIsola, pEvento->nSettore,szCDOPE,PQgetvalue(PGRes,0,0));
							} else {
								/* strano ... */
								sprintf(szMsgTxt,"ATTENZIONE : Isola:%d : Settore:%d : nessun operatore presente\n",
									pEvento->nIsola, pEvento->nSettore);
							}
							PQclear(PGRes);
						...
						if(bOperatore){
							/* se codice operatore corretto - entro in attesa badge */
							nAttesaBadge = TRUE;
							nOpBlinking  = 1;
						}
					}
					...
					sprintf(szMsgTxt,"ATTENZIONE : Isola:%d : Settore:%d : Lettura badge errata [%s]\n",
						pEvento->nIsola, pEvento->nSettore,pEvento->szBarcode);
					SendMessage(PROC_MAIN, PROC_SETTORI, DISPLAY_MSG, szMsgTxt);
					strcpy(szStatoLettura,"ERROR");
				}
				/* attualizzo */
				pSettore->nAttesaBadge = nAttesaBadge;
				pSettore->nOpBlinking  = nOpBlinking;
			} else {
				szCDOPE[0]='\0';
				strcpy(szStatoLettura,"NON GESTITO");
			}
			/* inserisco evento in badge_registry */
			PGRes=PGExecSQL(Cfg.nDebugVersion,"insert into badge_registry (badge_id,operatore_id,terminal_id,isola_id,settore_id,stato_lettura) values ('%s','%s',%d,%d,%d,'%s');",  
				pEvento->szBarcode, 
				szCDOPE,
				GetSettore(pEvento->nIsola, pEvento->nSettore), 
				pEvento->nIsola, 
				pEvento->nSettore,
				szStatoLettura);
			PQclear(PGRes);
...
void ReadOperatori(void)
{
	...
	*/
	for(nIndex=0;nIndex<Cfg.nNumeroSettori;nIndex++){
		strcpy(pSettori[nIndex].szCodOperatore,"0000");
	}

	PGRes=PGExecSQL(Cfg.nDebugVersion ,"select os.opcdope,os.opisola,os.opset,op.opnmope,op.opcgope from opeset as os,operatori as op where os.opcdope=op.opcdope;");
...
int ReadUbicazioni(PUBICAZIONI pUbi)
{
	...
	PGRes=PGExecSQL(FALSE,"select ubicazione, codprod, locazione, isola, settore, display, 0, 0, settore_can, riga_can, colonna_can, priorita, cnistato from ubicazioni where cancella=' ';");
	...
	for(nIndex=0;nIndex<nTuples;nIndex++){
		...
		pUbi->Ubicazione[nIndex].nIOS              = 0;
		pUbi->Ubicazione[nIndex].nCPU              = 0;
...
void SetLampada(int nIOS,int nCPU,int nModulo,int nRowIndex, int nColumn)
{
	char szBuffer[256];
	...
	*/
	if(nModulo==0 && nRowIndex==0 && nColumn==0){
#ifdef TRACE
		trace_out_vstr_date(1,"ERRORE - SetLampada(%d,%d,%d,%d,%d)",nIOS,nCPU,nModulo,nRowIndex,nColumn);
#endif
	} else {
		sprintf(szBuffer,"%d,%d,%d,%d,%d", nIOS,nCPU,nModulo,nRowIndex,nColumn);
...
void ResetLampada(int nIOS,int nCPU,int nModulo,int nRowIndex, int nColumn)
{
	char szBuffer[256];
	...
	*/
	for(nIndex=0;nIndex<Cfg.nNumeroSettori;nIndex++){
		if(pSettori[nIndex].nIsola==nIsola && pSettori[nIndex].nSettore==nSettore){
			...
			*/
			PGRes=PGExecSQL(FALSE,"select display, 0, 0, settore_can, riga_can, colonna_can from ubicazioni where isola='%d' AND settore='%4d' order by priorita;",nIsola,nSettore);
			...
			pSettori[nIndex].nRigProg=0;                 /* Progressivo Riga - RM 30-11-2019  */
...
BOOL UpdateRigaOrdine(PRIGA_PRELIEVO pRigaOrdine,PSETTORE pSettore)
{
	...
	switch(pRigaOrdine->nStato){
		...
		case ATTESA_PRELIEVO:
			...
			* 03-03-2008 RM : gestione per collo dal 2006 !!
			*/
			sprintf(szSQLCmd,"update rig_prod set rpstato='%c' where ordkey='%s' AND rpnmcol=%d AND rpcdubi='%s' and rpprogr=%d;",
				cStato,
				pRigaOrdine->szCodSped,
				pRigaOrdine->nNumCollo,
				pRigaOrdine->szCodUbi,
				pRigaOrdine->nRigProg);
		break;
		case PRELEVATA:
			...
			*/
			sprintf(szSQLCmd,"update rig_prod set rpstato='%c',rpcdopr='%s',rpdtpre='%s',rptmpre='%s',rptmprl='now'  where ordkey='%s' AND rpnmcol=%d AND rpcdubi='%s' and rpprogr=%d;",
				cStato,
...
#ifdef TRACE
	trace_out_vstr_date(1, "SIMULA_LUCE Settore %d",nSettore);
#endif
	for(nIndex=0;nIndex<Cfg.nNumeroSettori;nIndex++){
		if(pSettori[nIndex].nIsola==nIsola && pSettori[nIndex].nSettore==nSettore && pSettori[nIndex].nStatoLampada==1){
			/* simulo la pressione tasto */
			sprintf(szSendBuffer,"%d,%d,%d,%d,%d",
				pSettori[nIndex].nIOS,pSettori[nIndex].nCPU,pSettori[nIndex].nModulo,pSettori[nIndex].nRiga,pSettori[nIndex].nColonna);
...
int get_letture_badge(int nIsola, int nSettore)
{
	...
	PGRes=PGExecSQL(Cfg.nDebugVersion,"select badge_id from badge_registry where isola_id=%d and settore_id=%d and to_char(read_time,'YYYYMMDD')=to_char(CURRENT_TIMESTAMP,'YYYYMMDD');",nIsola, nSettore);
