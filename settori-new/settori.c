...
int main(int argc,char** argv)
{
	...
	while ((nOpt = getopt(argc, argv, "atli:s:")) != -1) {
		switch(nOpt){
			case 'a':	/* alone */
				Cfg.bAlone=TRUE;
			break;
	...
	if(nErr){
		...
		printf("Uso : %s [-a][-t][-l][-i isola][-s settore]\n",argv[0]);
		printf("\t-a          : Run Alone\n");
		printf("\t-t          : Run Test\n");
		printf("\t-l          : Loop Test\n");
	...
	*/
	if(Cfg.bAlone && bTest){
		...
		*/
		strcpy(szSQLCmd,"select ubicazione, codprod, cancella, locazione, isola, settore, display, ios, cpu, modulo, riga, colonna, priorita, cnistato from ubicazioni where ");
		if(nIsola!=-1){
			sprintf(szBuffer,"isola=%d ",nIsola);
			strcat(szSQLCmd,szBuffer);
		}
		if(nSettore!=-1){
			sprintf(szBuffer,"and settore=%d ",nSettore);
			strcat(szSQLCmd,szBuffer);
		}
		strcat(szSQLCmd,"order by isola,priorita;");
