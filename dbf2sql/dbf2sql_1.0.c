...
void cfg_create_index (CFGSTRUCT *cfgstruct, PDBSTRUCT pDB)
{
	...
	*/
	fprintf (stdout, "\nCREATE INDEX ON %s: ",cfgstruct->szTableName);
...
*/
{
	...
	while ((nOpt = getopt(argc, argv, "c:d:f:h:p:t:u:arv?")) != EOF) {
		switch(nOpt){
			case 'a': /* add to table */
...
void main(int argc,char **argv)
{
	...
	*/
	...
	/*
	* se non è presente il file .cfg ricavo la struttura
	* della tabella dal file .dbf
 	*if (cfgstruct.bDbfDataFile && !cfgstruct.bConfigFile){
	*	ReadFieldsStructFromDBF(dbf_fd, &DB);
	*}
	*/
