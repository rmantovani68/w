...
char *NormalizeString(char *szString)
{
	...
	*/
	...
	strcpy(szString,szDestination);
	return (szString);
}


/*
* BOOL FileEmpty(char *szFileName)
*/
...
void gtk_label_printf(GtkWidget *w,char *fmt,...)
{
	...
	if(w){
		...
		gtk_label_set_markup (GTK_LABEL(w), g_utf8_normalize(msgbuf,-1,G_NORMALIZE_DEFAULT)); 
	}
}
	...
	*/
	...
	if(nLen >= pDB->nRecordLen && !strncmp(szBuffer+pDB->nRecordTypePos,pDB->szRecordType,pDB->nRecordTypeLen)){
		...
		*/
		...
		szInsertSQLCmd[strlen(szInsertSQLCmd)-1]='\0';
			...
			*/
			...
			if(pDB->pFields[nFieldIndex]->nFieldFlag & IMPORT_MASK){
				...
				*/
				nIndex=0;
				while (szFieldString[nIndex]){
					if(!isascii(szFieldString[nIndex])){
						szFieldString[nIndex]='.';
					}
					nIndex++;
				}
