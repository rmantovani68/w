	...
	if(PQntuples(PGResCollo)){
		...
		*/
		...
		if(szStato[0]!=COLLO_EVASO && szStato[0]!=COLLO_SPEDITO_HOST){
			...
			*/
			PesaCollo(szOrdKey,nCollo,nPesoColloBilancia);
			...
			*/
			if(Cfg.GestionePesatura==PESATURA_COLLO){
				...
				*/
				...
				if(nPesoDiff>nTolleranza){
					/* Collo FUORI TOLLERANZA */	
					bColloConforme=FALSE;
					cColor=&cRED;
#ifdef TRACE
					trace_out_vstr_date(1,"Pesato Collo(%s - %d) fuori tolleranza [diff:%d toll:%d]",szOrdKey,nCollo,nPesoDiff,nTolleranza);
#endif
				} else {
					/* Collo IN TOLLERANZA */	
					bColloConforme=TRUE;
...
int CalcolaPesoCollo(char *szOrdKey,int nCollo)
{
	...
	if (nRigheCollo!=0){
		...
		*/
		...
		PGResCollo=PGExecSQL(Cfg.nDebugVersion,"select cpprgcl from col_prod where ordkey='%s' and cpnmcol=%d;",szOrdKey,nCollo);
...
/*
* BOOL EvadiCollo(char *szOrdKey,int nCollo)
...
#ifdef TRACE
	trace_out_vstr_date(1,"EvadiCollo [%s][%d]",szOrdKey,nCollo);
#endif

	GetTimeHHMMSS(time((long *)0),szTimeBuffer);
	GetDateYYYYMMDD(time((long *)0),szDateBuffer);

	PGResCollo=PGExecSQL(Cfg.nDebugVersion,"update col_prod set cpstato='%c',cpdteva='%s',cptmeva='%s' where ordkey='%s' AND cpnmcol = %d;",cStatoCollo,szDateBuffer,szTimeBuffer,szOrdKey,nCollo);
...
/*
* BOOL PesaCollo(char *szOrdKey,int nCollo,int nPeso)
...
#ifdef TRACE
	trace_out_vstr_date(1,"PesaCollo [%s][%d]",szOrdKey,nCollo,nPeso);
#endif

	GetTimeHHMMSS(time((long *)0),szTimeBuffer);
	GetDateYYYYMMDD(time((long *)0),szDateBuffer);

	PGResCollo=PGExecSQL(Cfg.nDebugVersion>2,"update col_prod set cpstato='%c',cppsrea=%d,cpdteva='%s',cptmeva='%s' where ordkey='%s' AND cpnmcol = %d;",cStatoCollo,nPeso,szDateBuffer,szTimeBuffer,szOrdKey,nCollo);
	if (!PGResCollo || PQresultStatus(PGResCollo) != PGRES_COMMAND_OK ) {
		bRetVal=FALSE;
#ifdef TRACE
		trace_out_vstr_date(1,"Fallito update su col_prod [%s - %d]",szOrdKey,nCollo);
...
BOOL CheckSottordineEvaso(char *szOrdKey)
{
	...
	if(bRetVal){
		...
		PGRes=PGExecSQL(FALSE,"select cpnmcol,cppsrea from col_prod where ordkey='%s' and cpstato='%c' and cpswlin!='1';",szOrdKey,COLLO_PESATO);

		if (PQresultStatus(PGRes)==PGRES_TUPLES_OK){

			if (PQntuples(PGRes)==nColliAutomatici){
				/*
				* tutti i colli AUTOMATICI del sottordine sono stati pesati
				*/
				char *pszColloKey;
				int nRigaListaColli=0;
				...
				*/
				PQclear(PGRes);
				PGRes=PGExecSQL(FALSE,"select cpnmcol,cppsrea from col_prod where ordkey='%s' and cpstato in ('%c','%c') and cpswlin!='1';",szOrdKey,COLLO_PESATO,COLLO_ELIMINATO);
...
int CalcPesoRealeOrdine(char *szOrdProg)
{
	...
	if (PQresultStatus(PGRes) == PGRES_TUPLES_OK){
		...
		PGResUpdate=PGExecSQL(FALSE,"update ric_ord set ropsrea=%d,rodteva='%s',rotmeva='%s' where ordprog='%s';",nPesoRealeOrdine,szDateBuffer,szTimeBuffer,szOrdProg);
		if (!PGResUpdate || PQresultStatus(PGResUpdate) != PGRES_COMMAND_OK ) {
...
BOOL SettaPesoSottordine(char *szOrdKey, int nPesoOrdine)
{
	...
	PGRes=PGExecSQL(FALSE,"update ord_prod set oppsrea=%d,opdteva='%s',optmeva='%s' where ordkey='%s';",nPesoOrdine,szDateBuffer,szTimeBuffer,szOrdKey);
	if (!PGRes|| PQresultStatus(PGRes) != PGRES_COMMAND_OK ) {
		bRetVal=FALSE;
