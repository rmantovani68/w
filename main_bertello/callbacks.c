...
void on_pbm_ricezione_tutto_activate(GtkMenuItem * menuitem, gpointer user_data)
{
	...
	if(FileExists(szOrdini)           || FileExists(szRighe)           || FileExists(szArticoli)      || FileExists(szUbicazioni) ||
       ...
       FileExists(szOrdiniBertello_n) || FileExists(szColliBertello_n) || FileExists(szRigheBertello_n)){
...
void do_storicizzazione_dati(GtkButton *button, gpointer user_data)
{
	...
	PGRes=PGExecSQL(Cfg.nDebugVersion,"create table if not exists ordini_bertello_backup as select * from ordini_bertello where false;"); PQclear(PGRes);
	PGRes=PGExecSQL(Cfg.nDebugVersion,"create table if not exists colli_bertello_backup  as select * from colli_bertello where false;");  PQclear(PGRes);
	PGRes=PGExecSQL(Cfg.nDebugVersion,"create table if not exists righe_bertello_backup  as select * from righe_bertello where false;");  PQclear(PGRes);
	...
	*/
	...
	for(nIndex=0;nIndex<nNumeroRecords;nIndex++){
		...
		*/
		...
		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"insert into ordini_bertello_backup select * from ordini_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);
		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"delete from ordini_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);

		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"insert into colli_bertello_backup select * from colli_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);
		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"delete from colli_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);

		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"insert into righe_bertello_backup select * from colli_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);
		PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"delete from righe_bertello where ordprog='%s';",PQgetvalue(PGRes,nIndex,0)); PQclear(PGResOrdini);
