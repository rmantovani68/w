...
BOOL AnalisiVolumetrica(void)
{
	...
	*/
	PGResOrdini=PGExecSQL(Cfg.nDebugVersion,"select ordprog,ronelab,roprgln,rostato,roflpdf,roflftz,roprgpr from ric_ord where (rostato='%c' OR rostato='%c') order by roprgln;",ORDINE_SPEDITO,ORDINE_SPEDITO_DA_EVADERE);
...
***************************************************************************************************/
			for(nRigaIndex=0,bFine=FALSE; nRigaIndex<nRigheOrdine && !bFine; nRigaIndex++){
				...
				*/
				...
				PGResCat=PGExecSQL(Cfg.nDebugVersion,"select prswffo, prqtpcf from articoli where prcdpro='%s';",Riga.szCodiceProdotto);
				...
				if(nCopiePCF){
					...
					* ------------------------------------
					* aggiungo un record all'archivio rig_prod per ogni riga 
					*/
					Collo.nCollo=0;
					if(!InsertRigProd(&Ordine,&Collo,&Riga,"rig_tmp",Ordine.cStatoRigaElaborata)){
						bFine=TRUE;
		...
		*/
		...
		for(nIsolaIndex=0;nIsolaIndex<nIsole;nIsolaIndex++){
			...
			if(!bPallet){
				...
				*/
				PGResRighe=PGExecSQL(Cfg.nDebugVersion,"select \
					...
					a.prlungh, a.prlargh, a.praltez, a.prswalt, a.prpesgr, a.prfcpcf,a.prperic\
			...
			*/
			PGResRighe=PGExecSQL(Cfg.nDebugVersion,"select \
				r.rpcdpro, r.rpqtspe, r.rpcdubi, r.rpswffo, r.rpswpcf,\
				a.prlungh, a.prlargh, a.praltez, a.prswalt, a.prpesgr, a.prfcpcf, a.prqtprs,a.prperic\
				from rig_tmp as r, articoli as a, ubicazioni as u \
				where r.ordprog='%s' and substr(r.rpcdubi,1,1)='%d' and a.prcdpro=r.rpcdpro and u.codprod=r.rpcdpro order by u.isola,u.priorita;",Ordine.szOrdProg,Ordine.nIsola);
			...
			for(nRigaIndex=0,bFine=FALSE;nRigaIndex<nRigheOrdine && !bFine;nRigaIndex++){
				...
				*/
				...
				} while (nRC==VOL_RIGA_SPEZZATA || nRC==VOL_CHIUDI_COLLO);
			}
			PQclear(PGResRighe);
			/*
		...
		if(nTuples){
			...
			if (nCollo){
				...
				if (nCollo==1 && Spedizione.cFlagPdf!=ORDINE_SENZA_PDF) {
					...
					* NB: importantissimo
					*/
					// Se il sotto-ordine ha un solo collo allora va nello stato incompleto terminato spedito
